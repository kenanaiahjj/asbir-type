...
class PackageAsbirSansReleaseTests(unittest.TestCase):
    def test_package_uses_format_folders_and_single_archive_root(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            packager.TARGET = root / 'release' / 'AsbirSans-1.1.1'
            packager.ARCHIVE = source / 'AsbirSans-1.1.1.zip'
            ...
            self.assertIn('AsbirSans-1.1.1/OTF/AsbirSans-Regular.otf', paths)
            self.assertIn('AsbirSans-1.1.1/TTF/AsbirSans-Regular.ttf', paths)
            self.assertIn('AsbirSans-1.1.1/Variable/AsbirSans-Variable.ttf', paths)
            self.assertIn('AsbirSans-1.1.1/README.md', paths)
